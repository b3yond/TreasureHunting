...
def show_foes(mission, mission_foes):  # returns a list of enemies at the beginning of the game
    ...
    for x in range(len(mission)):
        for y in range(len(mission[x])):
            if mission[x][y] == 1:
                i += 1
                result.append([i, str(mission_foes[1]), x, y, 0, 0])  # id, name, y-coordinate, x-coordinate, damage taken, stun
            elif mission[x][y] == 2:
                i += 1
...
def near(playerx, playery, id, foelist):
...
def attack(player, action, foelist, mission):  # processes a single attack
    ...
    if action == 0:
        ...
        if near(playerx, playery, id,
            ...
            if melee_skill(player.srg, player.ges) + randint(0, 50) > melee_skill(target.srg, target.ges) + randint(0,20):                                                                                                                    20):
                if randint(1, 100) > 10:
