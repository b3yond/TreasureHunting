...
def near(playerx, playery, id, foelist):  # tests if the enemy is near the player
    if playerx + 1 == foelist[id][3] or playerx - 1 == foelist[id][3] and playery + 1 == foelist[id][2] or playery - 1 == foelist[id][2]:
        return True
    else:
        return False
    ...
    for i in range(len(steps)):
        ...
        if steps[i] == "w":
            if mission[playery][playerx - 1] == "O":
                ...
                widgetlist[playery][playerx - 1] = w1  # player widgetlist entry
                ...
                widgetlist[playery][playerx] = w1
                # left_place widgetlist entry
            ...
            if mission[playery - 1][playerx] == "O":
                ...
                widgetlist[playery - 1][playerx] = w1  # player widgetlist entry
                mission[playery - 1][playerx] = 0  # player missionlist entry
                logo = PhotoImage(file="Images/0leer.gif")  # place leer at left place
                w1 = Button(battlefield, image=logo)
                w1.logo = logo
                w1.grid(row=playerx, column=playery)
                widgetlist[playery][playerx] = w1  # left_place widgetlist entry
                mission[playery][playerx] = "O"  # left place missionlist entry
                playery -= 1
    ...
    for y in range(len(mission)):
        ...
        for x in range(len(mission[y])):  # place the new field at .grid(row = x, column = y)
            if mission[y][x] == 0:
                ...
                try:
                    if str(mission_foes[str(mission[y][x])]) == "D":
