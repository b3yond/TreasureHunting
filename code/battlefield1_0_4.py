...
def show_foes(mission, mission_foes): # returns a list of enemies at the beginning of the game
    ...
    for y in range(len(mission)):
        for x in range(len(mission[y])):
            if mission[y][x] == 1:
                i += 1
                result.append(
                    [i, str(mission_foes[1]), y, x, 0, 0])  # id, name, y-coordinate, x-coordinate, damage taken, stun
            elif mission[y][x] == 2:
                i += 1
                result.append([i, str(mission_foes[2]), y, x, 0, 0])
            elif mission[y][x] == 3:
                i += 1
                result.append([i, str(mission_foes[3]), y, x, 0, 0])
            elif mission[y][x] == 4:
                i += 1
                result.append([i, str(mission_foes[4]), y, x, 0, 0])
            elif mission[y][x] == 5:
                i += 1
                result.append([i, str(mission_foes[5]), y, x, 0, 0])
            elif mission[y][x] == 6:
                i += 1
                result.append([i, str(mission_foes[6]), y, x, 0, 0])
            elif mission[y][x] == 7:
                i += 1
                result.append([i, str(mission_foes[7]), y, x, 0, 0])
            elif mission[y][x] == 8:
                i += 1
                result.append([i, str(mission_foes[8]), y, x, 0, 0])
            elif mission[y][x] == 9:
                i += 1
                result.append([i, str(mission_foes[9]), y, x, 0, 0])
            elif mission[y][x] == "E":
                i += 1
                result.append([i, str(mission_foes["E"]), y, x, 0, 0])
            elif mission[y][x] == "V":
                i += 1
                result.append([i, str(mission_foes["V"]), y, x, 0, 0])
            elif mission[y][x] == "R":
                i += 1
                result.append([i, str(mission_foes["R"]), y, x, 0, 0])
...
def near(playerx, playery, id, foelist): # tests if the enemy is near the player
    if playerx + 1 == foelist[id][3] or playerx - 1 == foelist[id][3] and playery + 1 == foelist[id][
        2] or playery - 1 == foelist[id][2]:
        return True
    else:
        return False
...
def dead(player, id, foelist, target, mission): # tests if an enemy is dead and removes him
    if foelist[id][4] >= target.kon * 8 + target.srg * 5 + 10:
        ...
        mission[y][x] = "O"  # erase enemy from missionlist
        widgetlist[y][x].grid_remove() # erase enemy from GUI
...
def attack(player, action, foelist, mission): # processes a single attack
    ...
    if action == 0:
        ...
        if foelist[id][1] == "werewolf2":
            target = Werewolf2()
        if near(player.playerx, player.playery, id,
...
def walk(playerx, playery, steps, mission, mission_foes): # walk from player coordinates to the point where steps[] directs
    for i in range(len(steps)):
        time.sleep(0.2)
        if steps[i] == "w":
            if mission[playery][playerx-1] == "O":
                widgetlist[playery][playerx].grid_remove() # remove player from GUI
                widgetlist[playery][playerx-1].grid_remove() # remove target from GUI
                logo = PhotoImage(file= "Images/player.gif") # place player at target
                w1 = Button(battlefield, image = logo)
                w1.logo = logo
                w1.grid(row=playerx-1, column=playery)
                widgetlist[playery][playerx-1].append(w1) # player widgetlist entry
                mission[playery][playerx-1] = 0 # player missionlist entry
                logo = PhotoImage(file= "Images/0leer.gif") # place leer at left place
                w1 = Button(battlefield, image = logo)
                w1.logo = logo
                w1.grid(row=playerx, column=playery)
                widgetlist[playery][playerx].append(w1) # left_place widgetlist entry
            ...
            if mission[playery-1][playerx] == "O":
                widgetlist[playery][playerx].grid_remove() # remove player from GUI
                widgetlist[playery-1][playerx].grid_remove() # remove target from GUI
                logo = PhotoImage(file= "Images/player.gif") # place player at target
                w1 = Button(battlefield, image = logo)
                w1.logo = logo
                w1.grid(row=playerx, column=playery-1)
                widgetlist[playery-1][playerx].append(w1) # player widgetlist entry
...
def your_turn(player, foelist, mission, mission_foes):
    ...
    if move == True:
        print("Wohin willst du gehen? du hast %s Schritte." % (str(player.movement)))
        s = [input("wasd: ")]
        s.lower()
        steps = []
            ...
            if near(playerx, playery, foelist[i], foelist) == True:
                ...
                while shots >= 0:
                    if directcombat == False:  # is the enemy in sight line? then you can shoot him:
                        print(str(foelist[i] + 1) + ": schiessen")
                        action = input()
                        attack(player, action, foelist, mission)
        ...
        for x in range(len(mission[y])):  # place the new field at .grid(row = x, column = y)
            if mission[y][x] == 0:
                ...
                try:
                    ...
                    pass
            widgetlist[y].append(w1)
    return playerx, playery, widgetlist  # return the player coordinates
...
def show_legend():
    ...
    w1.logo = logo
    w1.grid(row=1, column=0)
    w1 = Label(legend, text="Deckung").grid(row=1, column=1)
...
# character creation
#show_legend()
playerx, playery, widgetlist = build_battlefield(chapter_one, chapter_one_foes)
